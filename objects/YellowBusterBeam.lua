---@class YellowBusterBeam : Sprite
---@overload fun(...) : YellowBusterBeam
local YellowBusterBeam, super = Class(Sprite)

function YellowBusterBeam:init(red, x, y, tx, ty, after)
...
function YellowBusterBeam:update()
    ...
    if Utils.dist(self.x, self.y, self.target_x, self.target_y) <= 40 then
        ...
        for i = 1, 8 do
            local burst = YellowBusterBurst(self.red, self.target_x, self.target_y, math.rad(45 + ((i - 1) * 90)), i > 4)
    ...
    if self.afterimg_timer >= 1 then
        ...
        local sprite = Sprite(self.red and "effects/rudebuster/beam_red" or "effects/rudebuster/beam_yellow", self.x, self.y)
